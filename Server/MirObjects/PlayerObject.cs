    ...
    public sealed class PlayerObject : MapObject
    {
        ...
        private string LogOutReason(byte reason)
        {
            switch (reason)
            {
                ...
                case 0:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: Server closed"), Name);
                case 1:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: Double login"), Name);
                case 2:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: Chat message too long"), Name);
                case 3:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: Server crashed"), Name);
                case 4:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: Kicked by admin"), Name);
                case 5:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: Maximum connections reached"), Name);
                case 10:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: Wrong client version"), Name);
                case 20:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: User gone missing / disconnected"), Name);
                case 21:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: Connection timed out"), Name);
                case 22:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: User closed game"), Name);
                case 23:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: User returned to select char"), Name);
                default:
                    return string.Format(Envir.Tr("{0} Has logged out. Reason: Unknown"), Name);
            }
        }
        ...
        private void StartGameSuccess()
        {
            ...
            SMain.Enqueue(string.Format(Envir.Tr("{0} has connected."), Info.Name));
        ...
        private void RefreshLevelStats()
        {
            ...
            switch (Class)
            {
                ...
                case MirClass.Assassin:
                    MaxHP = (ushort)Math.Min(ushort.MaxValue, 14 + (Level / Settings.ClassBaseStats[(byte)Class].HpGain + Settings.ClassBaseStats[(byte)Class].HpGainRate + Level / 20F) * Level);
                    MaxMP = (ushort)Math.Min(ushort.MaxValue, ((3 + Level / 12F) * Level) + (Level * Settings.ClassBaseStats[(byte)Class].MpGainRate));
                    break;
                case MirClass.Archer:
                    MaxHP = (ushort)Math.Min(ushort.MaxValue, 14 + (Level / Settings.ClassBaseStats[(byte)Class].HpGain + Settings.ClassBaseStats[(byte)Class].HpGainRate) + Level / 20F * Level);
                    MaxMP = (ushort)Math.Min(ushort.MaxValue, (5 + Level / 10F) * Level + (Level * Settings.ClassBaseStats[(byte)Class].MpGainRate));
                    break;
                case MirClass.Monk:
                    MaxHP = (ushort)Math.Min(ushort.MaxValue, 14 + (Level / Settings.ClassBaseStats[(byte)Class].HpGain + Settings.ClassBaseStats[(byte)Class].HpGainRate + Level / 20F) * Level);
                    MaxMP = (ushort)Math.Min(ushort.MaxValue, 3.3F * Level + (Level * Settings.ClassBaseStats[(byte)Class].MpGainRate));
                    break;
            }
        ...
        private void RefreshSkills()
        {
            for (int i = 0; i < Info.Magics.Count; i++)
            {
                ...
                switch (magic.Spell)
                {
                    ...
                    case Spell.Strong:
                        MaxHP = (ushort)Math.Min(ushort.MaxValue, MaxHP + Level * 3 + (magic.Level + 1) * 10);
                        break;
                    #endregion
        ...
        public void Chat(string message)
        {
            ...
            if (GMLogin)
            {
                if (message == GMPassword)
                {
                    IsGM = true;
                    SMain.Enqueue(string.Format(Envir.Tr("{0} is now a GM"), Name));
                    ReceiveChat(Envir.Tr("You have been made a GM"), ChatType.System);
                    Envir.RemoveRank(Info);//remove gm chars from ranking to avoid causing bugs in rank list
                }
                else
                {
                    SMain.Enqueue(string.Format(Envir.Tr("{0} attempted a GM login"), Name));
                    ReceiveChat(Envir.Tr("Incorrect login password"), ChatType.System);
                }
                GMLogin = false;
                return;
            }
            ...
            else if (message.StartsWith("@"))
            {
                ...
                switch (parts[0].ToUpper())
                {
                    ...
                    case "MOB":
                        ...
                        if (mInfo == null)
                        {
                            ReceiveChat((string.Format(Envir.Tr("Monster {0} does not exist"), parts[1])), ChatType.System);
                            return;
                        }
                        ...
                        ReceiveChat((string.Format(Envir.Tr("Monster {0} x{1} has been spawned."), mInfo.Name, count)), ChatType.System);
                        break;
                    ...
                    case "GIVEGOLD":
                        ...
                        SMain.Enqueue(string.Format(Envir.Tr("Player {0} has been given {1} gold"), player.Name, count));
                        break;
                    ...
                    case "GIVECREDIT":
                        ...
                        SMain.Enqueue(string.Format(Envir.Tr("Player {0} has been given {1} credit"), player.Name, count));
                        break;
                    case "GIVESKILL":
                    ...
                    case "INFO":
                        {
                            ...
                            switch (ob.Race)
                            {
                                case ObjectType.Player:
                                    ...
                                    ReceiveChat(string.Format(Envir.Tr("Name : {0}, Level : {1}, X : {2}, Y : {3}"), plOb.Name, plOb.Level, plOb.CurrentLocation.X, plOb.CurrentLocation.Y), ChatType.System2);
                                    break;
                                case ObjectType.Monster:
                    ...
                    case "CREATEMAPINSTANCE": //TEST CODE
                        ...
                        if (map == null)
                        {
                            ReceiveChat(string.Format(Envir.Tr("Map {0} does not exist"), parts[1]), ChatType.System);
                            return;
                        }
        ...
        public void RangeAttack(MirDirection dir, Point location, uint targetID)
        {
            ...
            if (target != null)
            {
                ...
                //todo:弓箭手体系修改
                //int damage = GetAttackPower(MinMC, MaxMC); //基础伤害(源码实现)
                int damage = GetAttackPower(MinDC, MaxDC); //基础伤害(修改为计算攻击)
                damage = (int)(damage * Math.Max(1, (distance * 0.35)));//range boost //距离伤害系数,越远伤害越高.
                damage = ApplyArcherState(damage); //精神加成
                int chanceToHit = 60 + (Focus ? 30 : 0) - (int)(distance * 1.5); //命中
            ...
            for (int i = 0; i < cell.Objects.Count; i++)
            {
                ...
                switch (spell)
                {
                    ...
                    case Spell.HalfMoon:
                        ...
                        break;
                    case Spell.JinGangGunFa:
                        magic = GetMagic(Spell.JinGangGunFa);
                        LevelMagic(magic);
                        break;
                    case Spell.CrossHalfMoon:
        ...
        private bool ElementalShot(MapObject target, UserMagic magic)
        {
            if (HasElemental)
            {
                ...
                //todo:弓箭手体系修改:万斤闪改MC为DC
                int damage = magic.GetDamage(GetAttackPower(MinDC, MaxDC) + orbPower);
        ...
        private bool DelayedExplosion(MapObject target, UserMagic magic)
        {
            ...
            //todo:弓箭手体系修改:修改爆闪物理伤害
            //增加攻魔分离体系,计算物攻伤害
            int archerACpower = magic.GetDamage(GetAttackPower(MinAC, MaxAC));


            //DelayedAction action = new DelayedAction(DelayedType.Magic, Envir.Time + delay, magic, power, target);
            DelayedAction action = new DelayedAction(DelayedType.Magic, Envir.Time + delay, magic, power, target, archerACpower);//向队列添加物攻数值
            ActionList.Add(action);
            return true;
        }
        ...
        private void CompleteMagic(IList<object> data)
        {
            ...
            switch (magic.Spell)
            {
                ...
                case Spell.Entrapment:
                    value = (int)data[1];
                    target = (MapObject)data[2];
                    //todo:技能修改:战士技能修改:擒龙手修改为可以对玩家施放.
                ...
                case Spell.PetEnhancer:
                    ...
                    //MinAC = (ushort)Math.Min(ushort.MaxValue, MinAC + buff.Values[1]);
                    //MaxAC = (ushort)Math.Min(ushort.MaxValue, MaxAC + buff.Values[1]); 
                    //int dcInc = 2 + target.Level * 2; //Values[0]
                    //int acInc = 4 + target.Level;     //Values[1]
                    int dcInc = (magic.Level + 2) * 2;  //Values[0]
                    int acInc = 0;     //Values[1] 同上,不用重复计算
                    //todo:技能修改:道士技能修改:血龙水持续时间修改,最后改回默认时间
                    ...
                    //target.AddBuff(new Buff { Type = BuffType.PetEnhancer, Caster = this, ExpireTime = Envir.Time + value * 100 + 6 + magic.Level, Values = new int[] { dcInc, acInc }, Visible = true });
                    LevelMagic(magic);
                    break;
                ...
                case Spell.DelayedExplosion:
                    ...
                    value = (int)data[1];
                    int archerACvalue = (int)data[3];//读取物理伤害
                    ...
                    if (target.Attacked(this, archerACvalue, DefenceType.AC, false) > 0) LevelMagic(magic);//将防御力改为物理防御
        ...
        public override int Attacked(PlayerObject attacker, int damage, DefenceType type = DefenceType.ACAgility, bool damageWeapon = true)
        {
            ...
            if (ElementalBarrier)
                ...
                damage -= damage * (ElementalBarrierLv + 3) / 10;//技能修改:金刚术技能防御数值修改
        ...
        public void CombineItem(ulong fromID, ulong toID)
        {
            ...
            Report.ItemCombined(Envir.Tr("CombineItem"), tempFrom, tempTo, indexFrom, indexTo, MirGridType.Inventory);
        ...
        public void SpellToggle(Spell spell, bool use)
        {
            ...
            switch (spell)
            {
                ...
                case Spell.DaMoGunFa:
                    ...
                    DaMoGunFa = true;
                    DaMoGunFaTime = Envir.Time + 10000;
       ...
       public void TalkMonster(uint objectID)
            ...
            if (CanTrade)
            {
                for (int p = 0; p < 2; p++)
                {
                    ...
                    for (int i = 0; i < TradePair[p].Info.Trade.Length; i++)
                    {
                        ...
                        Report.ItemMoved("TradeConfirm", u, MirGridType.Trade, MirGridType.Inventory, i, -99, string.Format(Envir.Tr("Trade from {0} to {1}"), TradePair[p].Name, TradePair[o].Name));
                    }

                    if (TradePair[p].TradeGoldAmount > 0)
                    {
                        Report.GoldChanged("TradeConfirm", TradePair[p].TradeGoldAmount, true, string.Format(Envir.Tr("Trade from {0} to {1}"), TradePair[p].Name, TradePair[o].Name));
        ...
        public void UnSummonIntelligentCreature(IntelligentCreatureType pType, bool doUpdate = true)
        {
            ...
            for (int i = 0; i < Pets.Count; i++)
            {
                ...
                if (doUpdate) ReceiveChat((string.Format(Envir.Tr("Creature {0} has been dismissed."), ((IntelligentCreatureObject)Pets[i]).CustomName)), ChatType.System);
        ...
        public void GameshopBuy(int GIndex, byte Quantity)
        {
            ...
            ReceiveChat(Envir.Tr("Your purchases have been sent to your Mailbox."), ChatType.Hint);
        }
