    ...
    public class MonsterObject : MapObject
    {
            ...
            for (int i = 0; i < Buffs.Count; i++)
            {
                ...
                switch (buff.Type)
                {
                    ...
                    case BuffType.PetEnhancer:
                        ...
                        //todo:技能修改:血龙水技能修改,宠物属性增加实现.
                        var cg = MinDC * buff.Values[0] / 50;
                        MinDC = (ushort)Math.Min(ushort.MaxValue, MinDC + MinDC * cg);
                        MaxDC = (ushort)Math.Min(ushort.MaxValue, MaxDC + MaxDC * cg);
                        MinAC = (ushort)Math.Min(ushort.MaxValue, MinAC + MinAC * cg);
                        MaxAC = (ushort)Math.Min(ushort.MaxValue, MaxAC + MaxAC * cg);
                        break;
                }
        ...
        private void ProcessBuffs()
        {
            ...
            for (int i = Buffs.Count - 1; i >= 0; i--)
            {
                ...
                #region 修复血龙水客户端不消失的bug
                //todo:技能修复:道士技能修复:血龙水特效不消失的bug
                if (buff.Visible)
