    ...
    public sealed class LoginScene : MirScene
    {
            ...
            Version = new MirLabel
                {
                    ...
                    Parent = _background,
                    Text = string.Format(CMain.Tr("Version: {0}"), Application.ProductVersion),
                };
        ...
        public override void Process()
        {
            if (!Network.Connected && _connectBox.Label != null)
                _connectBox.Label.Text = string.Format(CMain.Tr("Attempting to connect to the server.\nAttempt:{0}"), Network.ConnectAttempt);
        }
        ...
        private void ChangePassword(S.ChangePasswordBanned p)
        {
            ...
            MirMessageBox.Show(string.Format(CMain.Tr("This account is banned.\n\nReason: {0}\nExpiryDate: {1}\nDuration: {2:#,##0} Hours, {3} Minutes, {4} Seconds"), p.Reason,
        ...
        private void Login(S.Login p)
        {
            ...
            {
                case 0:
                    MirMessageBox.Show(CMain.Tr("Logging in is currently disabled."));
                    _login.Clear();
                    break;
                case 1:
                    MirMessageBox.Show(CMain.Tr("Your AccountID is not acceptable."));
                    _login.AccountIDTextBox.SetFocus();
                    break;
                case 2:
                    MirMessageBox.Show(CMain.Tr("Your Password is not acceptable."));
                    _login.PasswordTextBox.SetFocus();
                    break;
                case 3:
                    MirMessageBox.Show(CMain.Tr("The AccountID does not exist."));
                    _login.PasswordTextBox.SetFocus();
                    break;
                case 4:
                    MirMessageBox.Show(CMain.Tr("Incorrect Password and AccountID combination."));
