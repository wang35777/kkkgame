...
namespace Client.MirScenes
{
    public class SelectScene : MirScene
    {
        ...
        public void StartGame()
        {
            if (!Libraries.Loaded)
            {
                MirMessageBox message = new MirMessageBox(string.Format(CMain.Tr("Please wait, The game is still loading... {0:##0}%"), Libraries.Progress / (double)Libraries.Count * 100), MirMessageBoxButtons.Cancel);

                message.BeforeDraw += (o, e) => message.Label.Text = string.Format(CMain.Tr("Please wait, The game is still loading... {0:##0}%"), Libraries.Progress / (double)Libraries.Count * 100);
        ...
        private void DeleteCharacter()
        {
            ...
            MirMessageBox message = new MirMessageBox(string.Format(CMain.Tr("Are you sure you want to Delete the character {0}?"), Characters[_selected].Name), MirMessageBoxButtons.YesNo);
