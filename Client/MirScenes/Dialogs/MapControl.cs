...
namespace Client.MirScenes.Dialogs
{
    public sealed class MapControl : MirControl
    {
        ...
        private bool onLeftButtonDown(MirDirection direction)
        {
            ...
            if (CMain.Shift)
            {
                if (CMain.Time > GameScene.AttackTime && CanRideAttack()) //ArcherTest - shift click
                {
                    ...
                    if (GameScene.DoubleSlash && (!User.HasClassWeapon && User.Weapon > -1)) return true;
        ...
        private void UseMagic(ClientMagic magic)
        {
            ...
            {
                GameScene.Scene.OutputMessageIfPrepared(string.Format(CMain.Tr("You cannot cast {0} for another {1} seconds."), magic.Spell.ToString(), ((magic.CastTime + magic.Delay) - CMain.Time - 1) / 1000 + 1));
                User.ClearMagic();
                return;
            }
