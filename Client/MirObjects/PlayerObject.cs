...
namespace Client.MirObjects
{
    public class PlayerObject : MapObject
    {
        ...
        public long BlizzardStopTime, ReincarnationStopTime, SlashingBurstTime, TianLeiZhenTime;
            ...
            if (TransformType > -1)
            {
                ...
                switch (CurrentAction)
                {
                    ...
                    case MirAction.JumpDown:
                        Frames.Frames.TryGetValue(MirAction.JumpDown, out Frame);
                        break;
                }
        ...
        private void CaclculateMovingOffset()
        {
            switch (CurrentAction)
            {
                case MirAction.Walking:
        ...
        {
            if (NextAction != null && !GameScene.CanMove && IsMoveAction(NextAction.Action))
                return;
            ...
            else
            {
                ...
                ProcessFrame(action);
                UpdateFrame(action, olddirection);
        ...
        private void ProcessFrame(QueuedAction action)
        {
            switch (CurrentAction)
            {
                case MirAction.Walking:
                    ...
                    else if (Class == MirClass.Assassin && Sneaking)
        ...
        private void DrawAction(QueuedAction action)
        {
            switch (CurrentAction)
            {
                case MirAction.Pushed:
        ...
        private void UpdateNextAction(QueuedAction action)
        {
            ClientMagic magic;
            switch (CurrentAction)
            {
                ...
                case MirAction.JumpDown:
                    ...
                    if (!RidingMount)
                    {
                        ...
                        if (GameScene.JinGangGunFa)
                        {
                            if (TargetObject != null || GameScene.Scene.MapControl.CanHalfMoon(CurrentLocation, Direction))
                            {
                                magic = User.GetMagic(Spell.JinGangGunFa);
                                if (magic != null && magic.BaseCost + magic.LevelCost * magic.Level <= User.MP)
                                {
                                    Spell = Spell.JinGangGunFa;
                                    CurrentMagic = MagicMgr.Get(Spell.JinGangGunFa); 
                                }
                            }
                        }
                        ...
                        if (GameScene.DaMoGunFa)
                        {
                            if (TargetObject != null)
                            {
                                magic = User.GetMagic(Spell.DaMoGunFa);
                                if (magic != null)
                                {
        ...
        private void UpdateFrame(QueuedAction action, MirDirection olddirection)
        {
            ArcherLayTrap = false;
            ...
            switch (CurrentAction)
            {
                case MirAction.Pushed:
                    ...
                    break;
                case MirAction.Spell:
                    ProcessSpellAction(action, olddirection);
                    break;
            }
        }

        private void UpdateCurrentFrames(QueuedAction action)
        {
            Spell = (Spell)action.Params[0];
            switch (Spell)
            {
        ...
        private bool CheckCanJinGang()
        {
            ...
            return Spell == Spell.JinGangGunFa;
        }

        private void ProcessSpellAction(QueuedAction action, MirDirection olddirection)
        {
            ArcherLayTrap = false;
            Spell = (Spell)action.Params[0];
            switch (Spell)
        ...
        private void ProcessMove(QueuedAction action)
        {
            Point temp;
            switch (CurrentAction)
            {
        ...
        private static bool IsMoveAction(MirAction action)
        {
            switch (action)
            {
                case MirAction.Walking:
        ...
        public override void DrawEffects(bool effectsEnabled)
        {
            ...
            if (CurrentMagic != null)
                CurrentMagic.OnDrawEffect(this, CurrentAction);
            switch (CurrentAction)
            {
                case MirAction.Attack1:
                    switch (Spell)
        ...
        private void PlaySpellEffectAndSound()
        {
            switch (Spell)
            {
                #region FireBall
                ...
                case Spell.XiangLongGunFa:
                    //MapControl.Effects.Add(new Effect(Libraries.Magic2, 1700 + (int)Direction * 10, 9, 9 * FrameInterval, CurrentLocation));
                    Effects.Add(new Effect(Libraries.Magic4, 170 + (int)Direction * 16, 10, 9 * FrameInterval, this));
                    SoundManager.PlaySound(20000 + (ushort)Spell * 10);
                    TianLeiZhenTime = CMain.Time + 2000;
                    break;
